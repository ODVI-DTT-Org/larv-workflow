...
@test "runtime_gate validates expected VM URL" {
    run bash -c "source scripts/lib/runtime_gate.sh && runtime_gate_validate_url 'http://sandbox.example.com:9000/'"
    [ "$status" -eq 0 ]
}
...
@test "runtime_gate requires phase URL artifacts" {
    ...
    printf "http://sandbox.example.com:9000/\n" > "$TMP/docs/larv/03-design/mockup-url.txt"
    printf "http://sandbox.example.com:9500/\n" > "$TMP/docs/larv/docsite-url.txt"
    printf "http://sandbox.example.com:8000/\n" > "$TMP/docs/larv/07-runtime/sandbox-url.txt"
