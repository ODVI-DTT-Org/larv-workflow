...
6. For every user-facing slice, include seed-data obligations: multiple focused seeders, at least 10 realistic records per feature, seeded test credentials for all roles/personas needed for browser QA, permissions scopes, full-flow test paths, and the expected testing guide path under `docs/user-manual/testing/`.
...
- Run migrations and seeders so the sandbox has seeded records visible immediately.
- Ensure reviewers can use the normal login screen with seeded credentials for every role/persona needed to test the full app flow.
...
grep -qi "credentials" docs/user-manual/seed-data.md
...
- UI slice acceptance criteria that reference `docs/larv/03-design/visual-implementation-contract.md` and concrete mockup paths
