...
- Gallery: `attio-venture-html-effectiveness/`
- Primary templates:
...
Use the Attio Venture brown/tan brand treatment and the shared Attio CRM layout language. Do not invent a new theme when these templates cover the needed surface.
...
2. Create `docs/larv/redesigns/<timestamp>-attio-venture/`.
3. Write `design-source.md` with the exact Attio Venture source files used and why the selected CRM template matches the app.
...
5. Read the Attio Venture CRM templates and extract concrete implementation rules: layout, spacing, typography, tokens, table density, cards, badges, sidebar/header behavior, buttons, inputs, tabs, filters, modals/drawers, logo/favicon treatment.
...
10. Use Playwright or browser screenshots for desktop and mobile checks against the Attio Venture template direction. Fix obvious visual drift, broken layouts, inaccessible text, dead primary actions, and inconsistent navigation.
...
- Do not use a generic Tailwind dashboard. The implemented app must visibly match the Attio Venture HTML Effectiveness CRM source.
